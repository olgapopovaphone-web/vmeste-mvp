(function(){
  ...
  function ensureSection(data){
    ...
    root.classList.add('hasEventMedia');
    ...
    var content=root.querySelector('.eventContent');
    var accordion=content&&content.querySelector('.eventAccordion');
    if(accordion)accordion.insertAdjacentElement('beforebegin',section);
    else if(content)content.appendChild(section);
  ...
  function updateHero(){
    ...
    if(img){img.src=cover.url;img.alt='Обложка события'}else{
      hero.querySelector('.eventHeroPlaceholder')?.remove();
  ...
  function tile(item,index){
    ...
    return '<button type="button" class="eventMediaTile" data-media-index="'+index+'" aria-label="Открыть '+(item.media_type==='video'?'видео':'фото')+'">'+visual+(item.is_cover?'<span class="eventMediaCoverBadge">Обложка</span>':'')+'</button>'
  }
  function render(section){
    ...
    if(count)count.textContent=current.counts.total?countLabel(current.counts):'Общая медиатека участников';
  ...
  async function uploadFiles(eventId,files,section){
    ...
    var bad=files.find(function(f){return !allowed.includes(String(f.type||'').toLowerCase())||f.size>MAX_SIZE});
