(function(){
  ...
  function icon(name){
    ...
    if(name==='back')return `<svg ${common}><path d="M15 5 8 12l7 7"/></svg>`;
    if(name==='heart')return `<svg ${common}><path d="M20.8 4.8a5.5 5.5 0 0 0-7.8 0L12 5.9l-1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.4a5.5 5.5 0 0 0 0-7.8Z"/></svg>`;
    if(name==='compare')return `<svg ${common}><rect x="4" y="5" width="11" height="13" rx="2"/><rect x="9" y="2" width="11" height="13" rx="2"/></svg>`;
    ...
    if(name==='info')return `<svg ${common}><circle cx="12" cy="12" r="9"/><path d="M12 10v7M12 7h.01"/></svg>`;
    if(name==='chat')return `<svg ${common}><path d="M20 15a4 4 0 0 1-4 4H9l-5 3 1.5-5A8 8 0 1 1 20 15Z"/><path d="M8 12h.01M12 12h.01M16 12h.01"/></svg>`;
    return `<svg ${common}><path d="M4 7h7M15 7h5M4 17h5M13 17h7M11 4v6M9 14v6"/><circle cx="13" cy="7" r="2"/><circle cx="11" cy="17" r="2"/></svg>`;
  }
  ...
  function detailsPanel(data){
    ...
    return `<div class="eventAccordionPanel" data-event-panel="details" hidden><div class="eventDetailGrid"><div><span class="eventInfoLabel">ОРГАНИЗАТОР</span><strong>${eventEsc(data.creator?.display_name||'Организатор')}</strong></div><div><span class="eventInfoLabel">ДАТА И ВРЕМЯ</span><strong>${eventEsc(eventDateTime(ev.starts_at))}</strong></div><div><span class="eventInfoLabel">МЕСТО</span>${placeValue(ev)}</div></div>${locationBlock}${sourceBlock}</div>`;
  }
  ...
  function managementPanel(data){
    ...
    const ev=data.event,finished=ev.status==='finished';
    return `<div class="eventAccordionPanel" data-event-panel="management" hidden>${!finished?`<div class="eventManageBlock"><span class="eventInfoLabel">ПРИГЛАШЕНИЕ</span><button class="eventManagePrimary" id="event-invite-link">Получить ссылку-приглашение</button><div id="event-invite-box" class="inviteBox"></div></div>`:''}<div class="eventManageBlock"><span class="eventInfoLabel">СОБЫТИЕ</span>${finished?'<p class="muted">Событие завершено и находится в Хронике.</p>':'<button class="finishEvent" id="finish-event">Завершить событие</button>'}<button class="deleteEvent" id="delete-event">Удалить событие</button></div></div>`;
  }
  function accordionRows(data){
    ...
    return `<div class="eventAccordion">
      <button class="eventAccordionRow" data-event-toggle="details"><span class="eventRowIcon">${icon('info')}</span><span><strong>Детали</strong><small>Программа, место и информация</small></span><b>›</b></button>${detailsPanel(data)}
      <button class="eventAccordionRow" data-event-toggle="chat"><span class="eventRowIcon">${icon('chat')}</span><span><strong>Чат</strong><small>${chatNote}</small></span><b>›</b></button>${chatPanel(data)}
      ${data.is_creator?`<button class="eventAccordionRow" data-event-toggle="management"><span class="eventRowIcon">${icon('manage')}</span><span><strong>Управление</strong><small>Для организатора</small></span><b>›</b></button>${managementPanel(data)}`:''}
    </div>`;
  }
  async function bindTopPreferences(eventId){
    ...
    try{const state=await prefRaw('state',{event_id:eventId});like.classList.toggle('active',!!state.liked);compare.classList.toggle('active',!!state.compared)}catch{}
  ...
  window.renderEventDetail=function(data){
    ...
    const ev=data.event,finished=ev.status==='finished',date=eventDateParts(ev.starts_at);
    ...
    const description=ev.description?`<div class="eventDescription"><p class="eventDescriptionText">${eventEsc(ev.description)}</p><button id="event-description-more">Ещё <span>⌄</span></button></div>`:'';
    root.innerHTML=`
      <div class="eventHero">${cover}<button class="eventHeroCircle eventHeroBack" aria-label="Назад">${icon('back')}</button><div class="eventHeroActions"><button class="eventHeroCircle eventHeroLike" id="event-hero-like" aria-label="Нравится">${icon('heart')}</button><div class="eventCompareUnit"><button class="eventHeroCircle" id="event-hero-compare" aria-label="Сравнить">${icon('compare')}</button><span>Сравнить</span></div></div>${upload}<div class="eventHeroCopy"><span class="eventHeroEy">СОБЫТИЕ · ЛЮДИ · ГОРОД</span><h1>${eventEsc(ev.title)}</h1><p>${eventEsc(heroSubtitle(ev))}</p></div><div class="eventGalleryCount">1/1</div><div class="eventGalleryDots"><i class="active"></i></div></div>
      <div class="eventContent">${finished?'<div class="eventFinishedBadge">ЗАВЕРШЕНО</div>':''}<div class="eventFacts"><div class="eventFact"><span class="eventFactIcon">${icon('calendar')}</span><div><strong>${eventEsc(date.main)}</strong><small>${eventEsc(eventTimeRange(ev))}</small></div></div><div class="eventFact"><span class="eventFactIcon">${icon('pin')}</span><div>${ev.place_id?`<button class="eventFactPlace" data-event-place="${eventEsc(ev.place_id)}">${eventEsc(ev.location_name||'Место')} <b>›</b></button>`:`<strong>${eventEsc(ev.location_name||'Место не указано')}</strong>`}<small>${eventEsc(ev.address||'')}</small></div></div></div>${participantsStrip(data)}${rsvp}${organizerState}${description}<div id="event-invite-box-quick"></div>${accordionRows(data)}</div>`;
    bindEventDetail(data);bindRedesignedEvent(data);
  };
})();
