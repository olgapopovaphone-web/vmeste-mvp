(function(){
  ...
  function coverData(file){
    ...
    return new Promise(function(resolve){
      ...
      reader.onload=function(){
        ...
        img.onload=function(){
          try{
            var w=1200,h=400,scale=Math.max(w/img.width,h/img.height),sw=w/scale,sh=h/scale,sx=(img.width-sw)/2,sy=(img.height-sh)/2;
  ...
  document.addEventListener('lya:create-community-submit',async function(e){
    var d=e.detail||{},name=String(d.name||'').trim();
    if(!name)return;
    ...
    try{
      ...
      var group={
        ...
        members:[{id:u.id||'owner',display_name:u.name,role:'owner',is_owner:true}],
        created_at:new Date().toISOString()
      };
      var groups=readGroups();groups.unshift(group);localStorage.setItem(GROUPS_KEY,JSON.stringify(groups));
      ...
      if(typeof window.openCommunityDetail==='function')window.openCommunityDetail(id);
