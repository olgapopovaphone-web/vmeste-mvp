...
function eventMoney(ev){return ev.price_minor?Math.round(ev.price_minor/100)+' ₽':'Бесплатно'}
...
function closeEventView(){
  activeEventId=null;
  if(eventChatTimer){clearInterval(eventChatTimer);eventChatTimer=null}
  openView('calendar');
}
...
async function loadEventDetail(eventId){
  ...
  try{
    ...
    if(data.can_chat){await loadEventMessages(eventId);startChatPolling(eventId)}
...
function renderEventDetail(data){
  ...
  const upload=data.is_creator?`<input type="file" id="event-cover-input" accept="image/jpeg,image/png,image/webp" hidden><button class="coverEdit" id="event-cover-button">${ev.cover_url?'Сменить обложку':'Добавить обложку'}</button><div class="coverUploadStatus" id="cover-upload-status" hidden></div>`:'';
  const rsvp=data.is_creator?'':`<div class="eventSection"><div class="eventSectionHead"><div><span class="ey">МОЙ ОТВЕТ</span><h3>Вы пойдёте?</h3></div></div><div class="rsvp"><button data-rsvp="going" class="${data.my_status==='going'?'active':''}">Пойду</button><button data-rsvp="interested" class="${data.my_status==='interested'?'active':''}">Возможно</button><button data-rsvp="declined" class="${data.my_status==='declined'?'active':''}">Не смогу</button></div></div>`;
  ...
  const invite=data.is_creator?`<div class="eventSection"><div class="eventSectionHead"><div><span class="ey">ПРИГЛАШЕНИЕ</span><h3>Позвать людей</h3></div></div><div class="eventActions"><button class="primary" id="event-invite-link">Получить ссылку</button></div><div id="event-invite-box" class="inviteBox"></div></div>`:'';
  const chat=data.can_chat?`<div class="eventSection"><div class="eventSectionHead"><div><span class="ey">ЧАТ</span><h3>Разговор события</h3></div><button class="repeat" id="chat-refresh">Обновить</button></div><div class="chatBox"><div id="chat-messages" class="chatMessages"><p class="muted">Загружаю сообщения…</p></div><form id="chat-form" class="chatForm"><textarea id="chat-text" maxlength="2000" placeholder="Написать сообщение"></textarea><button>↑</button></form></div></div>`:`<div class="eventSection"><div class="eventSectionHead"><div><span class="ey">ЧАТ</span><h3>Разговор события</h3></div></div><div class="chatLocked">Чат открывается организатору и участникам со статусом «Пойду» или «Возможно».</div></div>`;
  root.innerHTML=`<div class="eventCover">${cover}${upload}</div><span class="ey">${data.is_creator?'МОЁ СОБЫТИЕ':'СОБЫТИЕ'}</span><h1 class="eventDetailTitle">${eventEsc(ev.title)}</h1>${ev.description?`<p class="muted">${eventEsc(ev.description)}</p>`:''}<div class="eventMeta"><div><small>КОГДА</small><b>${eventEsc(eventDateTime(ev.starts_at))}</b></div><div><small>ГДЕ</small><b>${eventEsc(ev.location_name||'Место не указано')}</b></div><div><small>ЦЕНА</small><b>${eventEsc(eventMoney(ev))}</b></div><div><small>ОРГАНИЗАТОР</small><b>${eventEsc(data.creator.display_name)}</b></div></div>${rsvp}<div class="eventSection"><div class="eventSectionHead"><div><span class="ey">УЧАСТНИКИ</span><h3>Кто будет</h3></div></div><div class="participantSummary"><span>Пойдут · ${counts.going}</span><span>Возможно · ${counts.interested}</span><span>Не смогут · ${counts.declined}</span></div><div class="participantList">${peopleHtml}</div></div>${invite}${chat}`;
...
async function prepareCover(file){
  ...
  const buffer=await blob.arrayBuffer();let binary='';const bytes=new Uint8Array(buffer);const chunk=0x8000;
  for(let i=0;i<bytes.length;i+=chunk)binary+=String.fromCharCode(...bytes.subarray(i,i+chunk));
  return {base64:btoa(binary),content_type:'image/jpeg'};
}

async function uploadEventCover(eventId,file){
  const status=document.querySelector('#cover-upload-status');if(!file)return;
  status.hidden=false;status.textContent='Готовлю изображение…';
